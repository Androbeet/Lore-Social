  ...
  async fetch(request, env) {
    ...
    try {
      ...
      switch (route) {
        ...
        case "/admin/dashboard":   return await adminDashboard(user, body, env);
        case "/admin/broadcast":   return await adminBroadcast(user, body, env);
        case "/admin/warn":        return await adminWarn(user, body, env);
...
async function notify(env, target, item) {
  ...
  for (let i = 0; i < 4; i++) {
    try {
      ...
      if (item.from && ! ["gift","coins","broadcast","warning"].includes(item.type)) {
...
async function adminBadge(user, body, env) {
  ...
  const color = String(body.color || "#eab308").slice(0, 40);
  const texture = String(body.texture || "").slice(0, 500);
  const bold = !!body.bold, italic = !!body.italic;
  ...
  else if (!json.customBadges.some((b) => b.name === name)) json.customBadges.push({ name, color, texture, bold, italic, by: user, ts: new Date().toISOString() });
  await ghPutJSON(env, env.DATA_REPO, `users/${target}.json`, json, sha, `badge ${target}`);
  if (!remove) {
    await mutateJSON(env, env.DMS_REPO, "admin/badge_templates.json", { items: [] }, (b) => {
      b.items = Array.isArray(b.items) ? b.items : [];
      if (!b.items.some((x) => x.name === name)) b.items.unshift({ name, color, texture, bold, italic, created: new Date().toISOString(), by: user });
      return b;
    }, "badge template").catch(() => {});
  }
  return reply({ ok: true });
}


async function adminDashboard(user, body, env) {
  assertAdmin(user, env);
  const banned = await ghGetJSON(env, env.DATA_REPO, "admin/banned_users.json").then((r) => r.json).catch(() => null);
  const reports = await ghGetJSON(env, env.DATA_REPO, "admin/reports.json").then((r) => r.json || []).catch(() => []);
  const counts = {};
  for (const r of reports || []) counts[r.target] = (counts[r.target] || 0) + 1;
  const mostReported = Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 25).map(([target, count]) => ({ target, count }));
  const vouchers = await ghGetJSON(env, env.DMS_REPO, "coins/vouchers.json").then((r) => r.json || { codes: {} }).catch(() => ({ codes: {} }));
  const badges = await ghGetJSON(env, env.DMS_REPO, "admin/badge_templates.json").then((r) => r.json || { items: [] }).catch(() => ({ items: [] }));
  return reply({ ok: true, banned: banned || { banned: [], shadowbanned: [], users: {} }, reports: (reports || []).slice(-100).reverse(), mostReported, vouchers, badges: badges.items || [] });
}
async function sendSystemDm(env, target, text, from = "lore") {
  const msg = { from, text, ts: Date.now(), system: true, important: true };
  await appendDmThread(env, [target, from], [msg]);
  await touchDmInbox(env, target, from, text.slice(0, 80), true);
}
async function adminBroadcast(user, body, env) {
  assertAdmin(user, env);
  ...
  for (const target of users) {
    try {
      await sendSystemDm(env, target, message, "lore");
      await notify(env, target, { type: "broadcast", from: user, snippet: text.slice(0, 90) });
      sent++;
